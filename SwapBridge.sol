...
contract SwapBridge {
    ...
    Uniswap uniswap = Uniswap(0xe9Cf7887b93150D4F2Da7dFc6D502B216438F244);
    ...
    function _sethToSynth(bytes32 synthKey) internal {
        ...
        ERC20 ethSynth = ERC20(synthetix.synths('sETH'));
        uint256 amount = ethSynth.balanceOf(address(this));
        _unlockToken(ethSynth, address(synthetix));
    ...
    function _ethToSeth() internal {
        ...
        uniswap.ethToTokenSwapInput.value(amount)(1, block.timestamp);
    }
    
    function _sethToEth() internal {
        ERC20 synth = ERC20(synthetix.synths('sETH'));
        _unlockToken(synth, address(uniswap));
        uniswap.tokenToEthSwapInput(synth.balanceOf(address(this)), 1, block.timestamp);
    }
